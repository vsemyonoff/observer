...
{
    template <typename T, template <typename> typename C = LastValue>
    class Signal;
    ...
    public:
        ...
        {
            static_assert(std::is_base_of<Observer, T>::value, "T must be derived from Observer");

            ScopedConnection c   = std::make_unique<Connection>(Callback::template bind<function>(instance));
        ...
        Connection* connect(T* instance)
        {
            ...
            ScopedConnection c   = std::make_unique<Connection>(Callback::template bind(instance));
            Connection*      ptr = c.get();
            insert(std::move(c));
        ...
        Connection* connect()
        {
            ScopedConnection c   = std::make_unique<Connection>(Callback::template bind<function>());
            Connection*      ptr = c.get();
            insert(std::move(c));
